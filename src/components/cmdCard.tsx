...
export default function CmdCard({ title, description, isRunning, args }: CmdCardProps) {
    ...
    args?.forEach(arg => {
        argElements.push(
            <div className="flex items-center justify-between" key={arg.name}>
    ...
    return (
        <div className={`${isRunning ? runningStyle : disableStyle} text-black shadow-lg rounded-lg p-4`}>
            <div className="flex items-center justify-between">
                <div className="flex items-center">
                    <div className="">
                        <h2 className="text-xl font-semibold text-gray-800">/{title}{isRunning ? "" : "(停止中)"}</h2>
